...
function EniNode({ id, data, selected, positionAbsoluteX }: { id: string; data: any; selected?: boolean; positionAbsoluteX?: number }) {
  ...
  return (
    <NodeTooltip name={data.name} type={data.type || 'ENI'} metrics={data.metrics}>
    <motion.div
      whileHover={{ 
        scale: 1.02, 
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex items-center justify-center shrink-0">
          <PlugsConnected size={28} weight="duotone" className="text-violet-500" />
        </div>
