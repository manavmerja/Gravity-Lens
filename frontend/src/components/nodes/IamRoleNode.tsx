...
import Icon from "../../../public/icons/amazon-iam.svg"
import { useCanvasStore } from '../../store/useCanvasStore';
import { Separator } from '../ui/separator';

const springTransition = { type: "spring", stiffness: 400, damping: 30 } as const;

function IamRoleNode({ id, data, selected, positionAbsoluteX }: { id: string; data: any; selected?: boolean; positionAbsoluteX?: number }) {
  ...
  return (
    <NodeTooltip name={data.name} type={data.type || 'IAM Role'} metrics={data.metrics}>
    <motion.div
      whileHover={{ 
        scale: 1.02, 
...
export default memo(IamRoleNode);
