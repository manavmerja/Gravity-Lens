...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        "hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-zinc-400 transition duration-200 hover:text-zinc-200 lg:flex lg:space-x-2",
        className
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarLogo = () => {
  return (
    ...
    >
      ...
      <span className="font-bold text-white text-lg tracking-wider">GravityLens</span>
    </Link>
  );
};
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    primary: "bg-white text-black shadow-[0_0_24px_rgba(255,255,255,0.3)] hover:bg-gray-200",
